...
parser.add_argument("--include_sx", type=str, default='true', help="if include sx in loss")
parser.add_argument("--include_sz", type=str, default='true', help="if include sz in loss")
parser.add_argument("--include_sxz", type=str, default='true', help="if include sxz in loss")
...
def run_experiments():
    for hparams_overwrite_list, seed in itertools.product(EXP_HPARAMS["params"], EXP_HPARAMS["seeds"]):
        ...
        config["seed"] = seed
        config['has_sx'] = True if args.include_sx == 'true' else False
        config['has_sz'] = True if args.include_sz == 'true' else False
        config['has_sxz'] = True if args.include_sxz == 'true' else False
